...
class DownloadLearnerApiViewMixin(object):
    """
    Requests text/csv data from the Learner Analytics API,
     and ensures that the REST framework returns it unparsed,
    ...
    def get_api_response(self, request, **kwargs):
        """
        Sets the HTTP_ACCEPT header on the request to tell the Learner Analytics API
        which format to return its data in.

        And tells the REST framework to render as text.
        NB: parent class must also define get_api_response()
        """
...
class LearnerListCsvView(DownloadLearnerApiViewMixin, LearnerListView):
    """
